...
#define SWITCH_STEPPER_MOVE_RANGE 380
...
StepperSwitchCtrl::StepperSwitchCtrl(   uint8_t motorPin1,
                                        uint8_t motorPin2,
                                        uint8_t motorPin3,
                                        uint8_t motorPin4) :
                                        _stepper(   8,
...
void StepperSwitchCtrl::calibrate()
{
    _stepper.run();
    switch(_state)
    {
...
void StepperSwitchCtrl::update(unsigned long curTime)
{
...
bool StepperSwitchCtrl::switchTo(uint8_t side)
{
    if(_state >= STATE_CALIBRATE_1)
    {
        return false;
